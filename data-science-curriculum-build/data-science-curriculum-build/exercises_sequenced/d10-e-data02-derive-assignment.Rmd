...
They output the same thing, but version 2 loads the data first and pipes it down the function as the first argument.
...
```
For the first summarize, all it does is to list a mean of all prices. For the second one, the function lists all combinations of color and clarity and computes the mean price of that.
...
cov_depth is on e-2 scale, while cov_diff is on e2 scale, there is 4 magnitudes of difference.
It could be that the diff_mean is very small to begin with, so a standard deviation of 2 would make the cov really large. 
