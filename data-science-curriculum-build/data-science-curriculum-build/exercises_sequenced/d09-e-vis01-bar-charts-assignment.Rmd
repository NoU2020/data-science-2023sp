...
**Observations**:
- we cant really see the individual data points of hwy for each cty value.
...
**Observations**:
- The legends are a bit unnecessary because each class is already labeled. we can tell that 2seaters and minivans are the least counted class, while suvs are the most counted. We dont know where or why they are counted and its a bit confusing. 
...
**Observations**:
-v2 further seperated each class on drv, which is seperated into 4, f and r (I have no idea what they stand for). The are certain classes with only a certain drv (like 2seater with r drv and minivan with f drv). The colors are humungo uggos tho. 

**Compare `v1` and `v2`**:

- Which visualization---`v1` or `v2`---enabled you to make more observations?
WELLLL, it depends on the goal of the plot. v2 provided more information of the drv of each class of car, but i still have no idea what the drives stand for and the colors look ugly. If the goal of the plot is to show case the different class of cars, v1 would be clearer, but if the goal of the plot is to show the distribution of drvs in specific classes, v2 is more effective.

- What was the difference between `v1` and `v2`?
v1 presented the class of the car using both columns and colors, v2 presented the class of car in columns and the drv distribution with colors.
...
**Observations**
- Most manufacturers choose to focus on less than 3 class of cars. SUV seems to be the most common one, the next one is midsize, compact and subbcompact. 
