...
df_q1 <- data.frame(x = runif(n), y = runif(n))
df_q1$inside <- ifelse(df_q1$x^2 + df_q1$y^2 <= 1, 1, 0)
df_q1$stat <- 4 * mean(df_q1$inside)
...
alpha <- 0.05
x_bar <- mean(df_q1$stat)
s <- 1

# Calculate t-score and margin of error
t_score <- qt(1 - alpha/2, df = n - 1)
moe <- t_score * s / sqrt(n)

# Calculate confidence interval
ci_lower <- x_bar - moe
ci_upper <- x_bar + moe

cat(sprintf("The %.0f%% confidence interval for pi is [%.5f, %.5f]\n", 100*(1-alpha), ci_lower, ci_upper))
```

    ## The 95% confidence interval for pi is [3.13200, 3.17120]
...
- What confidence level did you choose?
  - i chose a 95% confidence level
- Was your sample size $n$ large enough? Why do you say that?
  - The sample size wasn’t big enough as each time I ran the program it
    still generated fairly different values
