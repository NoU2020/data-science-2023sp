...
- aaaa theres a lot of variables
...
ggplot(df_psaap, aes(x = x, y = T_norm, color = as.factor(idx))) +
  geom_point() +
  geom_line()+
  labs(x = "x", y = "T_norm") +
  theme_minimal()
...
cat("Baseline model RMSE:", rmse_baseline, "\n")
```

    ## Baseline model RMSE: 0.2844778

``` r
cat("Cheat model RMSE:", rmse_cheat, "\n")
```

    ## Cheat model RMSE: 0.2317709

``` r
cat("Non-physical model RMSE:", rmse_nonphysical, "\n")
```

    ## Non-physical model RMSE: 0.3988128

**Observations**:

- Which model is *most accurate*? Which is *least accurate*?
  - The cheat model is the most accurate, the non-physical model is the
    least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is an output, it is probably an effective predictor for T norm
    because it is also an output value. the inputs of avg_T might be
    also associated with T_norm.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No, because it is also an output, therefore it is not a valid
    predictor.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is a metadata, it is the simulation run amount, which does not
...
- T_norm tends to increase as x increases, meaning location along the
  flow is a good predictor.
- T_norm also tends to increase as U_0 increases, this trend appears to
  be weaker than x.
- The relationship between T_norm and W or L is unclear.
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - q4 tends to be a bit more accurate, though i am uncertain by what
    margin. the dots are more aligned with the line in the q4 model
    graph than the x only graph
- Which model tends to be *more confident* in its predictions? Put
  differently, which model has *narrower prediction intervals*?
  - q4 has a more confident prediction as the prediction intervals are
    generally smaller
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - fit simple only has 1 predictor, while fit q4 has 4 predictors
...
- How much do you trust your model? Why?
  - With an r-squared value of 0.57, I would be cautious in trusting the
    model too much. This indicates that the model explains only 57% of
    the variability in the response variable, and there could be other
    factors that are not included in the model that affect the outcome.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - I would use a prediction interval for this task, because we want to
    predict a range of values for T_norm at the proposed design point,
    and we want to quantify the uncertainty around that prediction. A
    confidence interval would only tell us about the uncertainty in the
    mean response, not the uncertainty in an individual response.
- What fraction of validation cases lie within the interval you predict?
  How does this compare with `pr_level`? -0 percent, this means the
  model is way off of the pr_level and there was some error in
  implenting the model that i can not be bothered to fix right now.
