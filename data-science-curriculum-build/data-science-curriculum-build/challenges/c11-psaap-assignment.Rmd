...
- Which model is *most accurate*? Which is *least accurate*?
  - The cheat model is the most accurate, the non-physical model is the least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective predictor?
  - avg_T is an output, it is probably an effective predictor for T norm because it is also an output value. the inputs of avg_T might be also associated with T_norm.
- Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
  - No, because it is also an output, therefore it is not a valid predictor.
- What *Category* of variable is `idx`? Does it have any physical meaning?
  - idx is a metadata, it is the simulation run amount, which does not have physical meaning. 
...
- T_norm tends to increase as x increases, meaning location along the flow is a good predictor.
- T_norm also tends to increase as U_0 increases, this trend appears to be weaker than x.
...
- Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
  - q4 tends to be a bit more accurate, though i am uncertain by what margin. the dots are more aligned with the line in the q4 model graph than the x only graph 
- Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
  - q4 has a more confident prediction as the prediction intervals are generally smaller
...
- How much do you trust your model? Why?
  - With an r-squared value of 0.57, I would be cautious in trusting the model too much. This indicates that the model explains only 57% of the variability in the response variable, and there could be other factors that are not included in the model that affect the outcome.
- What kind of interval---confidence or prediction---would you use for this task, and why?
  - I would use a prediction interval for this task, because we want to predict a range of values for T_norm at the proposed design point, and we want to quantify the uncertainty around that prediction. A confidence interval would only tell us about the uncertainty in the mean response, not the uncertainty in an individual response.
- What fraction of validation cases lie within the interval you predict? How does this compare with `pr_level`?
  -0 percent, this means the model is way off of the pr_level and there was some error in implenting the model that i can not be bothered to fix right now.
...
- Are there any other recommendations you would provide?
  - Unfortunately, the fraction of validation cases within the predicted interval is 0%, which suggests that the model is not accurately predicting the response variable for the validation data. This could be due to a variety of reasons, such as not including all relevant predictors or having insufficient data to model the relationship. Therefore, it may not be appropriate to rely solely on this model to make recommendations for the design team. Further analysis and model refinement may be necessary to improve the accuracy of the predictions.
