...
- Document your observations here.
  - Other than a few counties, the distinction between 2person and 3person families compared to the rest are fairly obvious.
  - While for most counties the median income increases with the amount of people in the family, occasionally 6 person families rank between 4 and 5 person families
  - ...
- Can you confidently distinguish between household incomes in Suffolk county? Why or why not?
  - No, especially for the 4 person, 5 person and 6 person families, there are significant overlaps between the margin of errors that even though the margin of errors are fairly tight, I cannot confidently distinguish them
...
- What *overall* trend do you see between `SE` and population? Why might this trend exist?
  - Generally speaking, the more populous an area, the smaller SE is. It is possible that a larger data sample would cause the data to be more reflective of the actual values.
- What does this *overall* trend tell you about the relative ease of studying small vs large counties?
  - It might be easier to read data from larger counties as it is more likely we have a definitive answer/less overlaps due to small populations
...
```
```{r}
df_nonfamily <- df_income %>%
  select(
    Geography,
    contains("Geographic"),
    contains("median") & matches("Nonfamily households") & !contains("Annotation of")
  ) %>%
  mutate(across(contains("median"), as.numeric)) %>%
  glimpse()
...
```{r}
df_interim_male <- df_nonfamily %>%
    pivot_longer(
    names_pattern = "(Estimate|Margin of Error).*(Male householder.*)",
    names_to = c(".value", "category"),
  ...
  na.omit(df_interim_male)
```


```{r}
df_interim_female <- df_nonfamily %>%
    pivot_longer(
    names_pattern = "(Estimate|Margin of Error).*(Female householder.*)",
    names_to = c(".value", "category"),
    cols = -c("Geography", "Geographic Area Name")
  ) %>%
  rename(
  ...
  na.omit(df_interim_female)

```

```{r}
df_interim <- bind_rows(df_interim_male, df_interim_female) %>% 
  arrange(Geography)
...
```{r}
df_ppt <- right_join(df_pop, df_interim, by = c("Geography", "Geographic Area Name"))

df_ppt

df_ppt %>%
  ggplot(aes(x = category, y = income_estimate, group = category)) +
  geom_boxplot()  +
...
- In massachusetts, there is a pretty clear divide in income between 15 to 24 years and the other age groups. In most cases, the income different between 25 to 44 years and 45 to 64 years is not very large, meanin that people generally stabilze between 25 to 44 years and see some small increase when they reach 45 to 64 years. the only exception to this is Suffolk and maybe nantucket but its a small county.
